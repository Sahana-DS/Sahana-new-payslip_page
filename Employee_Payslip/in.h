    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (window.location.protocol === 'file:') {
                showNotification('error', 'This application must be run via the server. Please open http://18.204.219.108:3018/Employee_Payslip instead of the local file.');
            ...
            logoImg.addEventListener('error', function(event) {
                ...
                showNotification('error', 'Failed to load logo. Using placeholder image. Please ensure the server is running and access via http://18.204.219.108:3018/Employee_Payslip.');
                logoImg.src = 'https://placehold.co/180x100?text=Logo';
            ...
            async function viewPayslip() {
                ...
                if (validateForm()) {
                    ...
                    try {
                        const response = await fetch('http://18.204.219.108:3018/api/payslips?employeeId=' + employeeId + '&month=' + month + '&year=' + year);
                        if (!response.ok) {
